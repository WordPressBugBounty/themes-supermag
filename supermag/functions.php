...
if ( ! function_exists( 'supermag_get_theme_options' ) ) :
	...
	function supermag_get_theme_options() {
		...
		// Use cache only if not in Customizer.
		...
		if ( is_array( $supermag_get_theme_options ) ) {
			$cached_theme_options = array_merge( $supermag_default_theme_options, $supermag_get_theme_options );
		} else {
			$cached_theme_options = $supermag_default_theme_options;
		}
